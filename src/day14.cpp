...
//	docking program on the ferry, so the docking parameters aren't being correctly
//	initialized in the docking program's memory.
//	
//	After a brief inspection, you discover that the sea port's computer 
//	system uses a strange bitmask system in its initialization program. Although 
...
//	significant bit (2^0, that is, the 1s bit) on the right. The current bitmask
//	is applied to values immediately before they are written to memory: a 0 or
//	1 overwrites the corresponding bit in the value, while an X leaves the bit 
...
//	address space begins initialized to the value 0 at every address.) In the above example, only two values in memory are not zero - 101 (at address 7) and 64 (at address 8) - producing a sum of 165.
//	
...
//	with your ferry's docking program. It must be using version 2 of the decoder 
//	chip!
//	
...
static auto GenerateBitVariations(std::bitset<36> bits, std::size_t i, const std::vector<uint64>& indices, std::vector<std::bitset<36>>& masks)
{
	if (i == indices.size())
	{
	...
	bits[indices[i]] = 0;
	GenerateBitVariations(bits, i + 1, indices, masks);
...
struct VM
{
	...
	std::string							ActiveAddressMask;
	std::vector<uint64>					FloatingBits;
	std::bitset<36>						Mask0;
	std::bitset<36>						Mask1;
	uint8								bVersion2 : 1;

	VM(bool bIsVersion2 = false)
	{
		Mask0 = { 0 };
		Mask1 = { 0 };
		bVersion2 = bIsVersion2;
	}

	auto PrepAddress(std::bitset<36> addr, const std::string& mask)
	{
		for (std::size_t i = 0, j = mask.size() - 1; i < mask.size(); ++i, --j)
		{
			switch (mask[i])
			{
	...
	void					SetValue(uint64 address, uint64 value)
	{
		if (bVersion2)
		{
			std::vector<std::bitset<36>> masks;
			GenerateBitVariations(0, 0, FloatingBits, masks);

			// Set with this mask
			const std::bitset<36> v = value;
			const auto bare = PrepAddress(address, ActiveAddressMask);

			for (const auto& mask : masks)
			{
				auto addr = mask | bare;
				Memory[addr.to_ullong()] = v.to_ullong();
			}
		}
		else
		{
			value &= ~Mask0.to_ullong();		// Clear with this mask
			value |= Mask1.to_ullong();		// Set with this mask
			Memory[address] = value;
		}
	}
};

static uint64	Solve_1(std::vector<std::string> input)
{
	uint64		answer = 0;
	VM			vm;
	...
	for (const std::string& line : input)
	{
		...
		{
			std::string mask = line.substr(7, line.size() - 7);

			// Be sure to reset the mask
			vm.Mask0.reset();
			vm.Mask1.reset();

			for (size_t n = 0; n < mask.size(); n++)
			{
				uint64	pos = mask.size() - n - 1;
		...
		{
			std::string address = line.substr(4, line.find(']') - 4);
			std::string value = line.substr(line.find('=') + 1);

			vm.SetValue(std::stoi(address), std::stoi(value));
		}
	}

	for (const auto& pair : vm.Memory)
	{
		answer += pair.second;
	}
	printf("Answer: %llu\n", answer);
...
static uint64	Solve_2(std::vector<std::string> input)
{
	uint64		answer = 0;
	VM			vm(true);
	...
	for (const std::string& line : input)
	{
		if (line.substr(0, 4) == "mask")
		{
			...
			for (size_t n = 0; n < mask.size(); n++)
			{
				...
				{
					vm.ActiveAddressMask = mask;
					vm.FloatingBits.push_back(pos);
				}
			}
		}
		else
...
void Day14::Part1()
{
	...
	Solve_1(GetStrInput("inputs/Day14.txt"));
}
...
void Day14::Part2()
{
	...
	Solve_2(GetStrInput("inputs/Day14.txt"));
}