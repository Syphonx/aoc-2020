...
//	
//	Because adapters can only connect to a source 1-3 jolts lower than its rating, in order 
//	to use every adapter, you'd need to choose them like this:
//	
...
//	how many different ways they can be arranged. Every arrangement needs to connect the 
//	charging outlet to your device. The previous rules about when adapters can successfully
//	connect still apply.
//	
...
//	(The charging outlet and your device's built-in adapter are shown in parentheses.) Given
//	the adapters from the first example, the total number of arrangements that connect the 
...
static uint64 Solve_2(std::vector<int32> input)
{
	...
	std::vector<int32>	oneJoltRunLengths;
	int32				contiguousCount = 0;
	for (int32 i = 0; i < adapters.size() - 1; i++)
	{
		if (adapters[i + 1].Joltage - adapters[i].Joltage == 1)
		{
			contiguousCount++;
		}
		else
		{
			...
			{
				oneJoltRunLengths.push_back(contiguousCount);
			}
			contiguousCount = 0;
		}
	}
	...
	answer = 1;
	int32		runCombinations[4] = { 1, 2, 4, 7 };
	for (size_t i = 0; i < oneJoltRunLengths.size(); i++)
	{
		answer *= runCombinations[oneJoltRunLengths[i]];
	}
...
void Day10::Part2()
{
	...
	Solve_2(GetIntInput("inputs/Day10.txt"));
}